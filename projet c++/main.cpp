...
void initialiser_champ(int M[m][n], int m, int n, int nb_mines)
{
	...
	do{
		i = int(((double)(rand())/RAND_MAX)*m);  //application random, rand_max définit dans stdlib!
		j = int(((double)(rand())/RAND_MAX)*n);
		if (M[i][j] != 9) {M[i][j] = 9; nb_mines--;}
	}while(nb_mines!=0);
	for(i=0;i<m;i++)
	{
		for(j=0;j<n;j++)
		{
/* 1 */		if(M[i][j]!=9 && M[i+1][j+1]==9 && j<n-1) M[i][j]++;	//	1	2	3  //
/* 2 */		if(M[i][j]!=9 && M[i+1][j]==9) M[i][j]++;				//  #	#	#  //
/* 3 */		if(M[i][j]!=9 && M[i+1][j-1]==9 && j>0) M[i][j]++;		//	   ---	   //
/* 4 */		if(M[i][j]!=9 && M[i][j+1]==9 && j<n-1) M[i][j]++;		//  4  |9|	5  //
/* 5 */		if(M[i][j]!=9 && M[i][j-1]==9 && j>0) M[i][j]++;		//	#  |9|	#  //
/* 6 */		if(M[i][j]!=9 && M[i-1][j+1]==9 && j<n-1) M[i][j]++;	//	   ---	   //
/* 7 */		if(M[i][j]!=9 && M[i-1][j]==9) M[i][j]++;				//	6	7	8  //
/* 8 */		if(M[i][j]!=9 && M[i-1][j-1]==9 && j>0) M[i][j]++;		//	#	#	#  //
		}
	}
}
/*========================================================================
...
/*========================================================================
  ...
  |-------|
=========================================================================*/
// PERMET d'afficher des infos à chaque affichage du tableau
void CLS(int nb, int ele, int coup){
		system("CLS");
		cout<<"__________________________________________________"<<endl;
		cout<<"Nombres de Mines en jeu : "<<nb<<endl<<endl;
		cout<<"Cases visibles : "<<ele<<endl<<endl;
		cout<<"Nombre de coup joue : "<<coup<<endl;
		cout<<"__________________________________________________"<<endl<<endl<<endl;
}
// une procédure pour quittez quand le jeu est terminer
void quitter(char menu){
		cout<<"Voulez-vous recommencer\? O/N ";
		do{cin>>menu; if(menu!='O' && menu!='o' && menu!='N' && menu!='n') cout<<"Mauvais caractere"<<endl;}while(menu!='O' && menu!='o' && menu!='N' && menu!='n');
		if(menu=='O' || menu=='o') {system("CLS");}
		if(menu=='N' || menu=='n') {exit(0);}
}
/*========================================================================
  |---|
...
=========================================================================*/
int main(int argc)
{	//permet de lancer l'application en fullscreen, grâce a l'appel de windows.h
    ...
    system("CLS"); //actualise la console, sinon la couleur reste sur "n" caractère imprimé
	...
	init_tab2Dbool(visible,m,n);
		...
		do{
			...
			do{cout<<endl<<"Entrez la ligne du carre a rendre visible : ";		cin>>l;}while(l<0 && l>m);
			do{cout<<endl<<"Entrez la colonne du carre a rendre visible : ";	cin>>c;}while(c<0 && c>n);
			coup++;
			if(champ[l][c]!=9)
			{
				decouvrir_carre(visible, champ, m, n, l, c);
				ele=nb_el_visible(visible, m, n);
				CLS(nb,ele,coup); //permettra d'afficher les infos en sorti de boucle, cad sur l'état gagnant
			}
			...
			cout<<"|----   |----  |----   |    |  |    | "<<endl;
			cout<<"|       |      |   \\   |    /  |    | "<<endl;
			cout<<"|       -----  |    \\  -----   ------ \a"<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl;
		  ...
		  cout<<endl<<endl<<endl<<endl<<"-------      /\\      -------   |\\    |   ------     *  * "<<endl;
			...
			cout<<"| |---|    /----\\    | |---|   |  \\  |   |-----     *  * "<<endl;
			cout<<"|     |   /      \\   |     |   |   \\ |   | "<<endl;
			cout<<"-------  /        \\  -------   |    \\|   ------     #  # \a \a"<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl;
			quitter(menu);
			}
	}while(m>0);
